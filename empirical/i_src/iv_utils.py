...
from i_src.ii_data import label2id, id2label
    ...
    def score_aspect(self):
        ...
        for i in range(self.data_num):
            golden_aspect_spans = get_aspects(
                self.goldens[i], self.sen_lengths[i], self.tokens_ranges[i]
            )
            for spans in golden_aspect_spans:
                golden_set.add(str(i) + "-" + "-".join(map(str, spans)))

            predicted_aspect_spans = get_aspects(
                self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i]
            )
        ...
        precision = (
            correct_num / len(predicted_set) if len(predicted_set) > 0 else 0
        )
