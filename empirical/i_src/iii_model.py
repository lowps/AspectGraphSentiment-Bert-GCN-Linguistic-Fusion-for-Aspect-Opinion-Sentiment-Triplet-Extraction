...
class RefiningStrategy(nn.Module):
    ...
    def forward(self, edge, node1, node2):
        ...
        edge_diag = (
            torch.diagonal(edge, offset=0, dim1=1, dim2=2)
            .permute(0, 2, 1)
            .contiguous()
        )
...
class GraphConvLayer(nn.Module):
    ...
    def __init__(
        self, device, gcn_dim, edge_dim, dep_embed_dim, pooling="avg"
    ):
    ...
    def forward(self, weight_prob_softmax, weight_adj, gcn_inputs, self_loop):
        ...
        weight_prob_softmax = weight_prob_softmax.permute(
            0, 2, 3, 1
        ).contiguous()
...
class Biaffine(nn.Module):
    def __init__(
        self, args, in1_features, in2_features, out_features, bias=(True, True)
    ):
    ...
    def forward(self, input1, input2):
        ...
        biaffine = biaffine.contiguous().view(
            batch_size, len2, len1, self.out_features
        )
        return biaffine
...
class EMCGCN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.post_emb = torch.nn.Embedding(
            args.post_size, args.class_num, padding_idx=0
        )
        self.deprel_emb = torch.nn.Embedding(
            args.deprel_size, args.class_num, padding_idx=0
        )
        self.postag_emb = torch.nn.Embedding(
            args.postag_size, args.class_num, padding_idx=0
        )
        self.synpost_emb = torch.nn.Embedding(
            args.synpost_size, args.class_num, padding_idx=0
        )

        self.triplet_biaffine = Biaffine(
            args, args.gcn_dim, args.gcn_dim, args.class_num, bias=(True, True)
        )
    ...
    def forward(
        ...
        self_loop = (
            ...
            .to(self.args.device)
            .unsqueeze(1)
            .expand(batch, 5 * self.args.class_num, seq, seq)
